...
raw: a relação entre as variáveis é crescente, ou seja, se aumentarmos a quantidade de docentes, aumenta-se a quantidade de artigos produzidos. como conf.low = 4.269955	e conf.high = 5.356786, se aumentarmos em 1 o número de docentes estaremos aumentando entre 4 e 5 o número de artigos produzidos.
raw: a reta do modelo cruza a nuvem de pontos exatamente no meio; portanto não há enviezamento, visto que ela não está mais acima ou mais abaixo de um conjunto de pontos;
raw: como o r² foi 0.814 (alto), isso signifca que o modelo baseado somente na quantidade de professores explica 81% da variância da quantidade de artigos. 
raw: o pvalor é bastante baixo, então a relação entre as variáveis não é ao acaso;
raw: o intervalo de confiança é positivo e não contém o zero, portanto a estimativa do coeficiente para as variáveis é significativa
...
```{r}
datatable(tidy(modelo2, conf.int = TRUE) %>% 
  select(-std.error), class = 'cell-border stripe')
```
...
raw: como interpreta esse gráfico doido? pelo gráfico vemos que quanto maior o número de doutores maior será o número de artigos produzidos. e quanto maior o número de docentes maior o número de artigos produzidos.
raw: o r² ainda deu bastante alto. isso signifca que o modelo baseado tbm no número de alunos explica 84% da variância da quantidade de artigos, apenas 3% maior que o modelo anterior. 
raw: o intervalo de confiança para número de alunos inclue o 0, portanto a estimativa para essa variável não é conclusiva / significativa. no entanto, para as outras variáveis, as estimativas é conclusiva.
raw: segundo o modelo: se aumentarmos em 1 o número de docentes estaremos aumentando entre 3 e 4 o número de artigos produzidos. se aumentarmos em 1 o número doutorandos estaremos aumentando entre 9 e 33 o número de artigos produzidos.
...
A produtividade será calculada pela divisão do número de artigos produzidos pela quantidade de docentes permanentes. 

```{r}
cacc = cacc %>%
  mutate(
    produtividade = producao / `Docentes permanentes`,
    `Tem doutorado` = as.factor(`Tem doutorado`),
    `Tem doutorado` = as.integer(`Tem doutorado`),
...
input <- cacc %>%
    #select(-produtividade, -Instituição, -Sigla, -Programa) %>% # dont include the variable we're predicting!
    select(Nível, `Tem doutorado`, `Docentes colaboradores`, `Docentes permanentes`, `Docentes visitantes`, `Resumos em conf`, `Resumos expandidos em conf`, `Artigos em conf`, Dissertacoes, Teses, `Outros Periodicos`, orientandos_proff) %>%
    as.matrix()
